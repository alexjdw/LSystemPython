...
class LSystem:
    rotation, char_pos, theta, width = 0, 0, 0, 1
    stack = []
    next_pos = cur_pos = np.zeros(2, float)
    ...
    def reset(self):
        ...
        self.cur_pos = self.next_pos = self.starting_pos
...
if __name__ == '__main__':
    ...
    LSystems = [LSystem(5, 5, 120, 'F>G>G', {'F': 'F>G<F<G>F', 'G': 'GG'}, (2/3, 1/4, 0)),  # sierpinski triangle
                ...
                LSystem(3, 5, 90, 'F', {'F': 'F>F<F<F<G>F>F>F<F', 'G': 'GGG'}, (1/2, 1/2, 90))     # square thingy
                ]
